...
模块化架构优化按 `docs/architecture/logic/modular_architecture_optimization_todo.md` 的 ARCH 子项逐项实施（ARCH-00 ～ ARCH-16 已完成）。实施中的新增契约测试、服务测试和集成测试必须接入默认入口 `tutorial_and_floors_test.gd`；计划中的测试在代码和默认入口落地前不得写成已执行。
