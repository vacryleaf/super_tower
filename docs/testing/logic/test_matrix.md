...
| RunContext/存档快照 | `run_context_persistence_test.gd` |
| 成长与奖励协调边界 | `run_progress_boundary_test.gd` |
| 教程/楼层/活动 | `campaign_test.gd` |
| 奖励和附着 | `reward_system_test.gd` |
| 存档迁移与序列化 | `persistence_test.gd` |
| UI 契约/意图边界 | `ui_contract_test.gd` |
| BattleTrace 诊断/事件断言 | `battle_trace_test.gd` |
...
模块化架构优化按 `docs/architecture/logic/modular_architecture_optimization_todo.md` 的 ARCH 子项逐项实施（ARCH-00 ～ ARCH-20 已全部完成）。实施中的新增契约测试、服务测试和集成测试必须接入默认入口 `tutorial_and_floors_test.gd`；计划中的测试在代码和默认入口落地前不得写成已执行。
