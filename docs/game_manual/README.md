...
首版职业包含战士和弓箭手。角色拥有完整装备栏，装备本身也是卡牌。装备不按强弱稀有度划分，只分普通装备与套装：普通装备提供更高基础数值，每个职业各自拥有 4 个 2 件套、4 个 3 件套、3 个 4 件套、3 个 5 件套和 2 个 6 件套，其中 6 件套包含 2/4/6 三档能力；同时提供不限职业的通用套装，包括 6 个 2 件套、5 个 3 件套和 4 个 4 件套。每个职业拥有多张专属技能卡，但单个角色在一局游戏中只能装备 4 个技能。塔内每层固定需要击败 10 只怪物。第 1 层是新手引导层，固定解锁基础装备和第 1 个技能；后续层数进入正式奖励循环。
...
6. 第 2 层开始，每次普通战斗胜利后从 3 个非装备卡奖励中选择 1 个，精英怪为 4 选 1，Boss 的非装备卡奖励为 5 选 1；战斗内抽取的附加卡主要提供伤害、确定暴击、确定躲避、减伤和资源修饰。
7. 装备卡只在 Boss 关卡的永久装备分支中出现；技能不会重复获取，永久技能全解锁后改为固定提供塔内技能。
8. 将奖励用于替换装备、调整技能，或融合附着到装备和技能上。
9. 击败当前层第 10 只怪物后进入下一层。
10. 角色死亡、主动退出或重新开始时，清空塔内临时增强、融合和 Buff。
...
- 本项目主要由 2D UI、行动按钮、附加卡、数据表和回合制逻辑组成，Godot 的 Control UI 系统、Resource 数据资源和信号机制适合快速迭代。
...
- [07_combat.md](07_combat.md)：回合制战斗、基础行动、附加卡、行动力、状态。
