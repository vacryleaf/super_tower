...
- 当前开发与验证平台：Windows、macOS。Linux、Android、iOS 和 Web 属于后续导出评估。
...
```json
{
  "kind": "attack",
  "label": "攻击 +5",
  "target": { "type": "equipment", "id": "training_sword" },
  "acquired_floor": 23,
  "value": 5,
  "run_id": "current_run"
}
...
- 当前装备实例。
- 当前 4 个技能槽。
- 当前局内技能背包，包括职业塔内技能和通用塔内技能；当前仍是后续设计。
- 当前装备和技能上的局内奖励附着。
- 当前基础行动附着，包括普通攻击、防御、躲避；当前仍是后续设计。
- 当前战斗状态卡抽取配置、当前持有状态卡、已使用记录和消耗记录。

新游戏或死亡时必须清空单局存档中的局内奖励附着。后续塔内技能和基础行动附着接入后，也必须按同一边界清空。
...
| RewardService | 生成普通、精英、Boss 局内奖励池，处理 Boss 永久装备分支、楼层数值和奖励显示短标签。 |
| RewardApplyService | 管理奖励选择、附着目标选择和奖励落地。 |
| CharacterService | 管理角色属性、职业资源、装备、技能和局内奖励附着。 |
| FusionSystem | 后续设计模块；若实现，负责装备、技能、基础行动的复杂融合目标和效果合并，不能替代当前奖励附着入口。 |
...
7. 实现普通怪 3 选 1、精英怪 4 选 1、Boss 3 个局内奖励加 1 个永久装备分支，并确保普通怪和精英怪不出现装备卡。
8. 后续实现 Boss 职业永久技能/塔内技能奖励，并保证职业永久技能和塔内技能都不重复获取。
9. 实现状态卡抽取，并限制为行动倍率、确定暴击、强化躲避、强化防御和紧急回撤等行动修饰。
10. 当前实现局内奖励附着到已装备装备和已装备技能；后续再实现塔内附着卡、技能卡融合和基础行动附着。
11. 实现永久存档和单局存档分离。
12. 实现敌人单位、战斗编队、特性、技能和普通/精英/Boss 数值成长。
13. 扩充首版内容表。
14. 做 Windows 与 macOS 的 1280x720 横屏 UI 验证；移动端横屏作为后续适配。

## 风险点

- 后续融合效果过多会导致规则解释复杂，应先限制效果类型。
- 装备栏很多，当前桌面横屏 UI 需要保持可读；未来移动端横屏 UI 仍需要分页，不能强行塞进一屏。
- 永久存档和单局存档必须分离，否则会破坏新游戏不继承局内奖励附着的设计。
