...
6. 新增敌人意图、先手和多段攻击时，优先修改 `EnemyActionRules`，保证真实 UI 战斗和自动模拟共用规则。
...
1. 核心规则拆分已经完成到当前原型所需边界：`PlaySession`、`RunSimulator`、`main.gd` 都只保留编排和兼容入口。
2. 新增战斗规则必须进 `BattleService`、`Combatant`、`EnemyActionRules` 或对应服务，不能直接写入 `PlaySession`。
