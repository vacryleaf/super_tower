...
状态：基础 Mod Loader 已实现；完整内容 Schema 和运行时切换仍待实现。
...
当前已实现 `ModLoader` 的目录扫描、manifest 校验、依赖解析、声明式 JSON 内容读取、命名空间/原版 ID 冲突保护、注册、禁用和结构化错误查询。Loader 只保留规范化字典，不执行 Mod 脚本；完整内容 Schema、跨领域引用、图鉴自动索引和 UI 启用/禁用仍列入后续任务。
